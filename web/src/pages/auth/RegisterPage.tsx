...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setRegisteredContact(data.email || data.mobile || '');
      setStep('otp');
    } catch (err: any) {
    ...
    >
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm">
          {step === 'form' ? 'Join 2,50,000+ students. Start free.' : `Enter the OTP sent to ${registeredContact}`}
        </p>
      </div>
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-5">
          <div>
            ...
            <input 
              ...
              placeholder="000000" 
              className="input text-center text-xl tracking-[0.5em] font-mono py-4" 
            />
          </div>

          <button type="submit" disabled={isLoading || otp.length !== 6} className="btn-primary w-full justify-center py-3">
            {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
          ...
          <p className="text-sm text-gray-500 text-center mt-4">
            ...
            <button type="button" onClick={handleResendOtp} className="text-primary-600 font-semibold hover:underline">
              Resend OTP
            </button>
          </p>
        </form>

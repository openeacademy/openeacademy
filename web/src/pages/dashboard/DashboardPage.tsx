...
import { BookOpen, Trophy, Bell, FileText, Calendar, ChevronRight, TrendingUp, Award, Crown } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const activePlan = dashboard?.activeSubscription?.plan;
  const hasPdfAccess = activePlan ? ['PREMIUM', 'EXAM_PACK', 'SUBJECT_PACK', 'PDF_ONLY', 'CUSTOM'].includes(activePlan.type) : false;
  const hasQuizAccess = activePlan ? ['PREMIUM', 'EXAM_PACK', 'SUBJECT_PACK', 'CUSTOM'].includes(activePlan.type) : false;
  ...
  return (
    <div className="max-w-7xl w-full space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {hasPdfAccess && <StatCard icon={FileText} label="PDFs Accessed" value={dashboard?.recentPDFs?.length || 0} color="text-primary-600" bg="bg-primary-50" />}
        {hasQuizAccess && <StatCard icon={Trophy} label="Quiz Attempts" value={dashboard?.recentAttempts?.length || 0} color="text-amber-600" bg="bg-amber-50" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {!hasPdfAccess && !hasQuizAccess && (
          <div className="card p-8 col-span-1 lg:col-span-2 text-center bg-gray-50 border border-dashed border-gray-200">
            <Crown className="w-12 h-12 text-amber-500 mx-auto mb-3" />
            <h2 className="text-xl font-bold text-gray-900 mb-2">Unlock Your Learning Journey</h2>
            <p className="text-gray-500 mb-5 max-w-md mx-auto">Subscribe to a plan to access premium study materials, mock tests, and track your progress.</p>
            <Link to="/subscriptions" className="btn-primary py-2.5 px-8">View Subscription Plans</Link>
          </div>
        )}
      </div>
